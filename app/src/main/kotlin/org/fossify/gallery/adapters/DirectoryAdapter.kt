...
class DirectoryAdapter(
    ...
    private fun renameDir() {
        if (selectedKeys.size == 1) {
            ...
            val renameStartedAt = System.currentTimeMillis()
            Log.d("RivuRename", "renameDir start sourcePath=$sourcePath")
            ...
            activity.handleLockedFolderOpening(sourcePath) { success ->
                Log.d("RivuRename", "locked folder check finished success=$success elapsed=${System.currentTimeMillis() - renameStartedAt}ms")
                if (success) {
                    val folderRenamed: (String) -> Unit = {
                        val callbackAt = System.currentTimeMillis()
                        Log.d("RivuRename", "folder rename callback newPath=$it elapsed=${callbackAt - renameStartedAt}ms")
                        activity.runOnUiThread {
                            ...
                            updateDirs(dirs)
                            Log.d("RivuRename", "UI update end duration=${System.currentTimeMillis() - uiStartedAt}ms")
                            ensureBackgroundThread {
                                try {
                                    val dbStartedAt = System.currentTimeMillis()
                                    Log.d("RivuRename", "directoryDB update start elapsed=${dbStartedAt - renameStartedAt}ms")
                                    activity.directoryDB.updateDirectoryAfterRename(firstDir.tmb, firstDir.name, firstDir.path, sourcePath)
                                    Log.d("RivuRename", "directoryDB update end duration=${System.currentTimeMillis() - dbStartedAt}ms")
                                    Log.d("RivuRename", "refreshItems requested elapsed=${System.currentTimeMillis() - renameStartedAt}ms")
                    ...
                    if (shouldUseAsyncFolderRename(dir.absolutePath)) {
                        showAsyncRenameFolderDialog(dir.absolutePath, renameStartedAt, folderRenamed)
                    } else {
                        RenameItemDialog(activity, dir.absolutePath, folderRenamed)
                    }
                }
            }
        } else {
    ...
    private fun shouldUseAsyncFolderRename(path: String): Boolean {
        val file = File(path)
        return file.isDirectory &&
            path.startsWith(activity.internalStoragePath) &&
            (!isRPlus() || isExternalStorageManager())
    }

    private fun showAsyncRenameFolderDialog(path: String, renameStartedAt: Long, callback: (newPath: String) -> Unit) {
        var ignoreClicks = false
        val view = DialogRenameItemBinding.inflate(activity.layoutInflater, null, false).apply {
            renameItemExtensionHint.beGone()
            renameItemName.setText(path.getFilenameFromPath())
        }
        ...
        activity.getAlertDialogBuilder()
            ...
            .apply {
                activity.setupDialogStuff(view.root, this, org.fossify.commons.R.string.rename) { alertDialog ->
                    ...
                    alertDialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                        ...
                        val newName = view.renameItemName.value
                        if (newName.isEmpty()) {
                        ...
                        if (!activity.getDoesFilePathExist(path)) {
                            activity.toast(String.format(activity.getString(org.fossify.commons.R.string.source_file_doesnt_exist), path))
                            return@setOnClickListener
                        }
                        ...
                        ignoreClicks = true
                        Log.d("RivuRename", "async folder rename accepted newPath=$newPath elapsed=${System.currentTimeMillis() - renameStartedAt}ms")
                        alertDialog.dismiss()
                        finishActMode()
                        activity.toast(R.string.renaming_folder)

                        // Rivu change: Commons runs the common folder rename path from the OK click.
                        // We keep Commons' storage logic, but move it off the UI thread for internal folders.
                        ensureBackgroundThread {
                            activity.renameFile(path, newPath, false) { success, _ ->
                                Log.d("RivuRename", "async folder rename finished success=$success elapsed=${System.currentTimeMillis() - renameStartedAt}ms")
                                if (success) {
                                    callback(newPath)
                                } else {
                                    activity.runOnUiThread {
                                        activity.toast(R.string.rename_folder_failed_trying_fallback)
                                        RenameItemDialog(activity, path, callback)
                                    }
                                }
                            }
                        }
    ...
    private fun setupView(view: View, directory: Directory, holder: ViewHolder) {
        ...
        bindItem(view).apply {
            ...
            if (lockedFolderPaths.contains(directory.path)) {
                ...
                if (directory.subfoldersMediaCount == 0 || directory.tmb.isBlank()) {
